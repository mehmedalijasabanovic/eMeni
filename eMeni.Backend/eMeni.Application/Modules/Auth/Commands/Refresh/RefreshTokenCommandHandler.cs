namespace eMeni.Application.Modules.Auth.Commands.Refresh;
    ...
    : IRequestHandler<RefreshTokenCommand, RefreshTokenCommandDto>
{
    public async Task<RefreshTokenCommandDto> Handle(RefreshTokenCommand request, CancellationToken ct)
    {
        ...
        if (rt is null || rt.ExpiresAtUtc <= nowUtc)
            throw new eMeniConflictException("Refresh token je nevažeći ili je istekao.");
        ...
        {
            throw new eMeniConflictException("Neispravan klijentski otisak.");
        }

        var user = rt.User;
        if (user is null )
            throw new eMeniConflictException("Korisnički nalog je nevažeći.");
